package name.sakanacatcher.recruit.auth.authentication.controller;
...
import name.sakanacatcher.recruit.auth.authentication.repository.ResourceIgnoreRepository;
import name.sakanacatcher.recruit.auth.authentication.repository.ResourceRepository;
...
public class ResourceController {
    ...
    public Result<Boolean> isIgnore(@RequestParam("url") String url){
        return new Result<Boolean>(SUCCESSFUL_CODE,SUCCESSFUL_MESG, ignoreRepository.findAllByUrlContainsAndEnableEquals(url,true).size() > 0);
    }
